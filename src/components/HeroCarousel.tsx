...
export default function HeroCarousel({ mode }: HeroCarouselProps) {
  ...
  return (
    <div className="relative overflow-hidden bg-[#FAF6F0] border-b border-[#E8DFC8]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 relative z-10">
        {slides.map((slide, index) => {
          ...
          return (
            <div
              key={index}
              className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10 animate-fade-in-slide"
            >
              {/* Slide Text Content */}
              <div className="lg:col-span-7 space-y-6 text-left">
                ...
                <h1 className="font-serif text-5xl sm:text-6xl lg:text-7xl font-semibold tracking-wide text-brand-charcoal leading-tight lowercase">
                  {slide.title1} <br />
                  <span className="text-brand-gold font-normal italic">{slide.title2}</span>
                </h1>
                
                <p className="font-sans text-xs sm:text-sm text-gray-500 max-w-lg leading-relaxed uppercase tracking-wider">
                  {slide.description}
                </p>
              ...
              <div className="lg:col-span-5 relative flex justify-center">
                ...
                <div className="relative w-full max-w-[340px] aspect-[3/4] border-2 border-brand-gold rounded-lg shadow-xl overflow-hidden bg-brand-cream-dark z-10 group">
                  <img
                    src={slide.image}
                    alt={slide.title1}
                  ...
                  <div className="absolute bottom-5 left-5 right-5 bg-white/95 backdrop-blur-xs p-4 rounded border border-[#E8DFC8] shadow-md transform translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
                    <span className="text-[8px] text-brand-gold uppercase tracking-widest font-sans font-bold block">Featured Look</span>
                    <span className="font-serif text-xs uppercase tracking-widest text-brand-charcoal font-semibold">{slide.title1} {slide.title2}</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <button
        onClick={handlePrev}
        className="absolute left-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-white/70 hover:bg-white border border-[#E8DFC8]/60 hover:border-brand-gold text-brand-charcoal hover:text-brand-gold transition-all cursor-pointer hidden md:flex items-center justify-center shadow-xs z-20"
      ...
      <button
        onClick={handleNext}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 p-2 rounded-full bg-white/70 hover:bg-white border border-[#E8DFC8]/60 hover:border-brand-gold text-brand-charcoal hover:text-brand-gold transition-all cursor-pointer hidden md:flex items-center justify-center shadow-xs z-20"
